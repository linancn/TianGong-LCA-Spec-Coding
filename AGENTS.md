...
- Use `uv run python scripts/kb/retrieve.py --query "<text>" --top-k 5` to sanity-check retrieval behaviour (hybrid/semantic search, metadata filters, reranking options) against the configured dataset before wiring flows into other stages.
