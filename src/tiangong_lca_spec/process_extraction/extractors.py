...
def _build_section_prompt() -> str:
    ...
    process_guidelines = (
        ...
        "state_purity=... | source_or_pathway=... | usage_context=...`. Populate every "
        "slot with rich bilingual synonyms, aliases/abbreviations, chemical identifiers, "
        "and state/source descriptors gathered from the paper or inferred from its "
        "context—never output placeholders such as `NA`/`N/A`. Append the original table "
        "reference, assumptions, or calculation notes after the template so downstream "
        "alignment retains provenance. High-use utilities (grid electricity, water, "
        "steam, oxygen, hydrogen, natural gas, etc.) must list at least two English and "
        "two Chinese descriptors or usage scenarios to maximise MCP recall."
    )
    module_guidelines = (
        ...
        "deciding which product, waste, and elementary flows to include. Examples "
        "include cut-off rules or systematic exclusions (e.g., neglecting "
        "infrastructure, cooling water, or auxiliary services).\n"
        "  * `dataSourcesTreatmentAndRepresentativeness."
        "referenceToDataSource`: cite literature references, reports, database "
        "records, or on-site documentation with author(s), publication year, and a "
        "persistent identifier (DOI, URL, accession). Do not use table titles or "
        "captions as standalone sources. Include original sources for converted "
        "datasets and note parameterisation references when mathematical models are "
        "used. For LCI results or partly terminated systems, list influential "
        "background data sources as well.\n"
        ...
        "or explain the best available descriptor instead of using placeholders. After "
