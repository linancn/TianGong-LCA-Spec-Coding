...
- **核心目录**：
  ...
  - `test/`：单元测试与回归测试数据。
- **协作接口**：标准工作流依赖 `.secrets/secrets.toml` 中配置的 OpenAI 与 tiangong LCA Remote 服务。首次接入时请先完成凭据校验，再批量运行 Stage 3+。
...
- **关键运行库**：`anyio`, `httpx`, `jsonschema`, `langgraph`, `mcp`, `openai`, `pydantic`, `pydantic-settings`, `python-dotenv`, `structlog`, `tenacity`。（`langgraph` 目前用于后续扩展图式工作流；现有发布的 CLI/编排器暂不依赖它。）
...
本地 TIDAS 校验依赖 CLI 命令 `uv run tidas-validate -i artifacts`，默认由 `scripts/md/stage3_align_flows.py` 自动执行（可用 `--skip-artifact-validation` 跳过）。该步骤不需要额外 MCP 凭据。
...
- 若需快速验证知识库检索效果，可运行 `uv run python scripts/kb/retrieve.py --query "<文本>" --top-k 10`，该 CLI 支持配置搜索方式、元数据过滤与 rerank 选项，方便在接入前做连通性检查。
...
- 在修改 Python 源代码后，按序执行：
  ...
  uv run pytest
  ```
- 若涉及流程抽取或对齐逻辑，优先跑一次最小化端到端流程（文献管线：Stage 1→Stage 4；JSON-LD 管线：Stage 1→Stage 3），命令示例与阶段要求见 `.github/prompts/extract-process-workflow.prompt.md`。
- 结构化日志默认使用 `structlog`，可在运行 CLI 时关注 `flow_alignment.*`、`process_extraction.*` 等事件以快速定位异常。
- 提交前请确认 `artifacts/` 下的中间产物未被误加入版本控制；必要时清理本地文件或补充 `.gitignore`。
