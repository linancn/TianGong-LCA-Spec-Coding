...
- **Stage 2 (Process generation)**: Codex drives the LLM in **two passes**. First, enumerate the complete process list (every table row / unit operation) with stable `processId`s. Second, loop over that list and generate each `processDataSet` individually, ensuring every `exchange` carries a complete `flowHints` object. Stage 2 is the single source of truth for exchange metadata; downstream stages will not auto-fill or repair missing hints. When enumerating candidates, treat “life cycle”, “scenario”, “system-level” and similar phrases as *risk flags* only: keep the process when the supporting table still lists physical LCI rows (units such as kg, g, t, m², m³, pcs, kWh, MJ). Drop it when the evidence shows nothing but LCIA indicators (ADP, AP, GWP, EP, PED, RI, etc.) with units such as “kg CO₂ eq.”, “kg SO₂ eq.”, “kg Sb eq.”, “kg PO₄³⁻ eq.”, “kg PM₂.₅ eq.”, or “MJ” explicitly labelled as impacts/depletion. Likewise, if the text merely says “inventory shown in Table 4” but the actual numeric rows are absent from `clean_text`, treat it as lacking quantitative LCI data and skip it.
